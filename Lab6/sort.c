...
int gpu_Sort(unsigned int *data, unsigned int length)
{
  ...
  unsigned int *out = (unsigned int *)malloc (length*sizeof(unsigned int));

  printf("GPU sorting.\n");
  
  io_data = clCreateBuffer(cxGPUContext, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, length * sizeof(unsigned int), data, &ciErrNum);
	
	out_data = clCreateBuffer(cxGPUContext, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, length * sizeof(unsigned int), out, &ciErrNum);
